...
NEURON {
    ...
    RANGE gion
    RANGE i__Kdr_pyr : a copy of the variable for current which makes it easier to access from outside the mod file
...
PARAMETER {
    ...
    n_forwardRate_rate = 0.040000003 (kHz) : was: 40.0 (per_time)
    n_forwardRate_midpoint = -24.9 (mV)    : was: -0.0249 (voltage)
    n_forwardRate_scale = 5 (mV)           : was: 0.005 (voltage)
    n_reverseRate_rate = 0.125 (kHz)       : was: 125.0 (per_time)
...
BREAKPOINT {
    ...
    ik = gion * (v - ek)
    i__Kdr_pyr =  -1 * ik : set this variable to the current also - note -1 as channel current convention for LEMS used!
